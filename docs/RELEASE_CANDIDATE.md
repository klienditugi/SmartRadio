...
- `verified` for llm, library, radio, and acquisition comes only from a stored test-connection row. Yaml and env `verify_status` are ignored, so an old example file that says `verified` reports `configured_unverified` (“configured but unverified, run test connection”) until test-connection is run again. Nothing is auto-promoted. A config or secret change invalidates the stored fingerprint. The fingerprint is HMAC-SHA256 keyed by `secrets/verification_hmac_key` (not the session secret). A missing or rotated key does not match, and the fingerprint is not returned.
...
| slskd search result → enqueue payload | Poll search responses, then rank in the isolated selector (`acquisition.selection`: junk paths, `min_file_size_mb` default 1, size cap, optional duration, broadcast `max_sample_rate` 48000 Hz and `max_bit_depth` 24, title tokens when a title is passed, requested version above a clean file above a penalized one, artist-in-path tiebreak, extension, peer, quality within those caps, typical size) → `{username,filename,size}` for `POST /transfers/downloads/{user}`. Files that omit sample rate or bit depth stay eligible. If the filters remove every candidate, the request fails as `no_suitable_result` with per-filter counts (`junk`, `min_file_size`, `title_mismatch`, and the rest) and nothing is enqueued. Zero responses stay `no usable search result`. The filename is the original string. Search responses have no id. |
