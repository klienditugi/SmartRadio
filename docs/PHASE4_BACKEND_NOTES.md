...
1. **Admin request transitions** — `packages/core` GRAPH: `REJECTED → RECEIVED|APPROVED`, `CLASSIFYING → RECEIVED`, `APPROVED → REJECTED`. Tests updated. `TERMINAL_STATUSES` still includes `REJECTED` (cannot cancel).
