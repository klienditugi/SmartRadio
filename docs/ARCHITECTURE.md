...
- Search selection is deterministic (`acquisition.selection`): junk paths (`._` basenames and `__MACOSX` segments), minimum size (default 1 mebibyte), size cap (default 200 MB), optional duration cap, broadcast sample-rate and bit-depth caps (default 48000 Hz and 24; files that omit those fields stay eligible), and title-token match when a request title is passed. Rank is extension, then a requested version above a clean file above a penalized one, then artist tokens in the path, then peer availability, quality within the caps, and typical size. If those filters remove every candidate, the selector returns no pick and does not relax them. The worker fails `QUEUED` → `FAILED` as `no_suitable_result` (with per-filter removal counts, including `junk`, `min_file_size`, and `title_mismatch`) and does not enqueue. Zero search responses stay `no usable search result`. Correlation is exact username + original filename + size, with a basename fallback only when that match is unique and the size agrees. Search responses have no id.
...
Filled settings that have not passed test-connection are `configured_unverified` (“configured but unverified, run test connection”) in doctor, the status API, and the dashboard. Worker jobs that need the integration fail with that message. `verified` is stored only as a test-connection row (integration, state, tested_at, config fingerprint). The fingerprint is one HMAC-SHA256 over a canonical serialization of the URL, user, model, and the secret value, keyed by `secrets/verification_hmac_key` (32 bytes, mode 600). That key is not the session secret, so rotating sessions does not force re-verification. The key and the fingerprint are not logged or returned. A missing or rotated key does not match any stored row. Yaml or env `verify_status`, including an old example file that says `verified`, is ignored. A matching `ready` row is the only verified state. Changing the URL, user, model, or secret changes the fingerprint and clears it. Nothing is auto-promoted. An acquisition install verified the old way shows `configured_unverified` until test-connection is run again. Status precedence is `not_configured`, then `configured_unverified` or a non-ready stored result, then probe states.
