...
Filled settings that have not passed test-connection are `configured_unverified` (“configured but unverified, run test connection”) in doctor, the status API, and the dashboard. Worker jobs that need the integration fail with that message. `verify_status` in yaml or env is not a source of truth. Nothing is auto-promoted to verified.
