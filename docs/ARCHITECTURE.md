...
`llm`, `library`, and `radio` `verify_status` default to `unverified`. Omitting the field, or copying `config/subwave.example.yaml`, does not read as verified. `verified` is stored only by that integration's `POST /api/v1/{llm,library,radio}/test-connection` after a read-only probe succeeds (Ollama `GET /api/tags` includes the configured model; Navidrome `GET /rest/ping` status ok; SUB/WAVE `GET /health` on-air plus `GET /dj/search`). Saving settings cannot set `verified`. An existing config with those settings filled in and no `verify_status` key is `configured_unverified` (“configured but unverified, run test connection”) in doctor, the status API, and the dashboard. Worker jobs that need the integration fail with that message. Nothing is auto-promoted to verified. Acquisition already follows the same rule: test-connection is the only writer of `acquisition.verify_status: verified`. The providers table has no SQL default; `syncProviders` copies these config values.
